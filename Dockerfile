...
WORKDIR /usr/src/mockserver_client
...
COPY Pipfile* /usr/src/mockserver_client/
...
ENV PROJECT_DIR=/usr/src/mockserver_client
...
COPY ./mockserver_client ${PROJECT_DIR}/mockserver_client
...
#CMD ["ddtrace-run", "uvicorn", "mockserver_client.api:app", "--host", "0.0.0.0", "--port", "5000", "--workers", "1", "--log-level", "debug"]
CMD ["uvicorn", "mockserver_client.api:app", "--host", "0.0.0.0", "--port", "5000", "--workers", "1", "--log-level", "debug"]

# Set the command to run the application using pipenv and Python without uvicorn and ddtrace-run
#CMD ["pipenv", "run", "python", "-m", "mockserver_client.api"]

# Start the REST API using FastMCP's built-in HTTP server
#CMD ["python", "mockserver_client/api.py"]
